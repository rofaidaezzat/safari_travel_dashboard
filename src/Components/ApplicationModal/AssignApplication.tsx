...
import { useAssignApplicationMutation } from "../../app/services/crudApplication";
...
export function AssignApplicationModal({
  open,
  application,
  onClose,
}: AssignApplicationModalProps) {
  const { toast } = useToast();
  const [assignApplication, { isLoading }] = useAssignApplicationMutation();
  ...
  useEffect(() => {
    if (application && open) {
       // If the application already has an assignedTo field, we might want to pre-select it
       // However, the interface says `assignedTo?: string`, so it might be undefined.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await assignApplication({ id: application._id, assignedTo }).unwrap();
      
      toast({
        title: "Application assigned",
        description: `Application has been successfully assigned.`,
      });
      onClose();
    } catch (error: any) {
      toast({
        ...
        description:
          error?.data?.message || "Could not assign the application. Please try again.",
      });
    }
  };
